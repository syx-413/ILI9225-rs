...
use lcd_ili9225_rs::{ColorOrder, Config, ILI9225, NoPin, Orientation, SpiMode};
...
fn main() {
    ...
    let mut display: ILI9225<MockSpi, NoPin, MockPin> =
        ILI9225::<MockSpi, NoPin, MockPin>::new_native_spi(spi, rst, config);
    display.init(&mut delay).unwrap();
    display.disable_display().unwrap();
    display.enable_display().unwrap();
    display.set_pixel(0, 0, 0x07E0).unwrap();
}
